...
test.auc <- data.frame(model=c("glm", "svm", "gbm", "cart", "rForest"),
                       auc=c(auc.glmModel, auc.svmModel, auc.gbmModel, auc.cartModel, auc.rfModel))
...
ggplot(aes(x=model, y=auc), data=test.auc) + geom_bar(fill = "light blue", stat="identity")
...
# plot(rfModel)
plot(roc.rfModel, print.auc=TRUE, print.auc.x=0.7, print.auc.y=0.1, print.auc.col="yellow", type="l", add=TRUE, col='yellow', lwd=1, lty=1)

legend("bottomright", legend=c("logistic regression", "SVM", "Boosted Trees", "CART", "rForest"), col=c("blue", "purple", "red", "green", "yellow"), lwd=1)
